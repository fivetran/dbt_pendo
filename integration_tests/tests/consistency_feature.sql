...
with prod as (
    select {{ dbt_utils.star(from=ref('pendo__feature'), except=exclude_cols) }}
    from {{ target.schema }}_pendo_prod.pendo__feature
),

dev as (
    select {{ dbt_utils.star(from=ref('pendo__feature'), except=exclude_cols) }}
    from {{ target.schema }}_pendo_dev.pendo__feature
),
