...
{{ adapter.dispatch('partition_by_source_relation', 'pendo') (has_other_partitions, alias) }}
...
{% macro default__partition_by_source_relation(has_other_partitions='yes', alias=None) -%}
    ...
    {%- if has_other_partitions == 'no' -%}
        {{ 'partition by ' ~ prefix ~ 'source_relation' if var('pendo_sources', [])|length > 1 }}
    {%- else -%}
        {{ ', ' ~ prefix ~ 'source_relation' if var('pendo_sources', [])|length > 1 }}
    {%- endif -%}
{%- endmacro -%}
